function varargout = VLA_3D_transformation(varargin)
% VLA_3D_TRANSFORMATION MATLAB code for VLA_3D_transformation.fig
%      VLA_3D_TRANSFORMATION, by itself, creates a new VLA_3D_TRANSFORMATION or raises the existing
%      singleton*.
%
%      H = VLA_3D_TRANSFORMATION returns the handle to a new VLA_3D_TRANSFORMATION or the handle to
%      the existing singleton*.
%
%      VLA_3D_TRANSFORMATION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VLA_3D_TRANSFORMATION.M with the given input arguments.
%
%      VLA_3D_TRANSFORMATION('Property','Value',...) creates a new VLA_3D_TRANSFORMATION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VLA_3D_transformation_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VLA_3D_transformation_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VLA_3D_transformation

% Last Modified by GUIDE v2.5 12-Jan-2015 15:28:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @VLA_3D_transformation_OpeningFcn, ...
                   'gui_OutputFcn',  @VLA_3D_transformation_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before VLA_3D_transformation is made visible.
function VLA_3D_transformation_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VLA_3D_transformation (see VARARGIN)

% Choose default command line output for VLA_3D_transformation
...
% UIWAIT makes VLA_3D_transformation wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VLA_3D_transformation_OutputFcn(hObject, eventdata, handles) 
...
if(nr==3)
    ...
    warndlg(' Input vector must have three rows. ');
    return;    
end
...
out1=sprintf(' norm(A)=%8.4g ',nA);
disp(out1);

disp(' ');
disp(' Transformed vector name:  output_vector');

assignin('base','output_vector', A);

...
//   rainflow_mean_max_min_mex.cpp  ver 1.7  April 6, 2016
...
void rainflow_cpp(double *L,double *C,double *AverageAmp,double *MaxAmp,
                     ...
                     double *D,double *ac1,double *ac2,
                     double *amean,double *amax, double *amin,
                     double *cL,double *y,double *dchoice,double *exponent,
                                                                size_t num)
{
...
//        mexPrintf("%8.4g \t %8.4g \n",Y,X);
  ...
  //         mexPrintf("p2 \t %8.4g \n",Y);
 	...
 	for(long i=0;i<kv;i++)
	{   
        amean[i]=(B[i][3]+B[i][2])/2.;
        amax[i]=max(B[i][3],B[i][2]);
        amin[i]=min(B[i][3],B[i][2]);
    }
...
//    mexPrintf(" ** nlhs=%d  nrhs=%d \n",nlhs,nrhs);
    ...
    plhs[0]= mxCreateDoubleMatrix(NB+1, 1, mxREAL);  // L
    plhs[1]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // C
    plhs[2]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // AverageAmp
    plhs[3]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // MaxAmp
    plhs[4]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // MinMean
    plhs[5]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // AverageMean
    plhs[6]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // MaxMean
    plhs[7]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // MinValley
    plhs[8]= mxCreateDoubleMatrix(NB, 1, mxREAL);    // MaxPeak
    plhs[9]= mxCreateDoubleMatrix(1, 1, mxREAL);     // D
    plhs[10]= mxCreateDoubleMatrix(MC, 1, mxREAL);   // ac1
    plhs[11]= mxCreateDoubleMatrix(MC, 1, mxREAL);   // ac2
    plhs[12]= mxCreateDoubleMatrix(MC, 1, mxREAL);   // amean
    plhs[13]= mxCreateDoubleMatrix(MC, 1, mxREAL);   // amax
    plhs[14]= mxCreateDoubleMatrix(MC, 1, mxREAL);   // amin 
    plhs[15]= mxCreateDoubleMatrix(1, 1, mxREAL);    // cL
    ...
    ac1         = mxGetPr(plhs[10]);
    ac2         = mxGetPr(plhs[11]);
    amean       = mxGetPr(plhs[12]);
    amax        = mxGetPr(plhs[13]);
    amin        = mxGetPr(plhs[14]);    
    cL          = mxGetPr(plhs[15]);
...
//    mexPrintf(" Call the rainflow_cpp function \n");
    ...
    rainflow_cpp(L,C,AverageAmp,MaxAmp,MinMean,AverageMean,MaxMean,MinValley,MaxPeak,D,ac1,ac2,amean,amax,amin,cL,y,dchoice,exponent,num);
}

...
% Last Modified by GUIDE v2.5 13-Aug-2014 10:34:11
...
set(handles.listbox_save_type,'Value',1);
...
if(units==1)
   out1=sprintf('Reduced %s Continuous Gust PSD, %s, Gust Velocity=%g ft/sec',out_model,out_axis,s);
   xlabel('Reduced Frequency (cycles/ft)');
   ylabel('PSD (ft/sec)^2/(cycles/ft)');  
else
   out1=sprintf('Reduced %s Continuous Gust PSD, %s, Gust Velocity=%g m/sec',out_model,out_axis,s);    
...
plot(freq,VPSD);
if(units==1)
    out1=sprintf('%s Continuous Gust PSD, %s, Gust Velocity=%g ft/sec',out_model,out_axis,s);
    ylabel('PSD (ft/sec)^2/Hz');    
else
    out1=sprintf('%s Continuous Gust PSD, %s, Gust Velocity=%g m/sec',out_model,out_axis,s);

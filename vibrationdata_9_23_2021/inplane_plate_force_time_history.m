...
vM_stress=zeros(num_elem,1);
Tr_stress=zeros(num_elem,1);
...
disp(' Element    Nodes      von Mises    Tresca  ');
...
%%%    Bcg
    ...
    sxx=element_cg_stress(i,:,1);
    syy=element_cg_stress(i,:,2);
    txy=element_cg_stress(i,:,3); 
    
    
    [vM,Tr,pstress1,pstress2]=von_Mises_Tresca_plane_stress(sxx,syy,txy);
    
    vM_stress(i)=std(vM);
    Tr_stress(i)=std(Tr);
            
    out1=sprintf(' %d  %d %d %d %d  %8.4g  %8.4g ',i,node1,node2,node3,node4,std(vM),std(Tr));
    disp(out1);
...
disp(' ');
if(iu==1)
    out1=sprintf(' Maximum Tresca Stress = %8.4g psi rms at elem %d',C,I);
else
    out1=sprintf(' Maximum Tresca Stress = %8.4g Pa rms at elem %d',C,I);    
end    

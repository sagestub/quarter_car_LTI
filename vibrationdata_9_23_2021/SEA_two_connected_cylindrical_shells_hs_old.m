function varargout = SEA_two_connected_cylindrical_shells_hs_old(varargin)
% SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD MATLAB code for SEA_two_connected_cylindrical_shells_hs_old.fig
%      SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD, by itself, creates a new SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD or raises the existing
%      singleton*.
%
%      H = SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD returns the handle to a new SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD or the handle to
%      the existing singleton*.
%
%      SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD.M with the given input arguments.
%
%      SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD('Property','Value',...) creates a new SEA_TWO_CONNECTED_CYLINDRICAL_SHELLS_HS_OLD or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SEA_two_connected_cylindrical_shells_hs_old_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SEA_two_connected_cylindrical_shells_hs_old_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SEA_two_connected_cylindrical_shells_hs_old

% Last Modified by GUIDE v2.5 01-May-2018 15:32:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SEA_two_connected_cylindrical_shells_hs_old_OpeningFcn, ...
                   'gui_OutputFcn',  @SEA_two_connected_cylindrical_shells_hs_old_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SEA_two_connected_cylindrical_shells_hs_old is made visible.
function SEA_two_connected_cylindrical_shells_hs_old_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SEA_two_connected_cylindrical_shells_hs_old (see VARARGIN)

% Choose default command line output for SEA_two_connected_cylindrical_shells_hs_old
...
% UIWAIT makes SEA_two_connected_cylindrical_shells_hs_old wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SEA_two_connected_cylindrical_shells_hs_old_OutputFcn(hObject, eventdata, handles) 
...
[D1,K1,S1,~,fring1]=honeycomb_sandwich_properties_wave(E,G,mu,tf,hc,diam1,rhof,rhoc);
[D2,K2,S2,~,fring2]=honeycomb_sandwich_properties_wave(E,G,mu,tf,hc,diam2,rhof,rhoc);
...
[rad_eff1,mph1]=...
    re_sandwich_cylinder_engine_alt(D1,K1,mu,diam1,L1,mpa,bc,mmax,nmax,gas_c,fcr,fring1,freq);

[rad_eff2,mph2]=...
    re_sandwich_cylinder_engine_alt(D2,K2,mu,diam2,L2,mpa,bc,mmax,nmax,gas_c,fcr,fring2,freq);
...
for i=1:num    
    ...
    [power1(i),power_dB1(i)]=power_from_spl_dB(fc(i),dB1(i),mph1(i),c,mpa,rad_eff1(i));    
    [power2(i),power_dB2(i)]=power_from_spl_dB(fc(i),dB2(i),mph2(i),c,mpa,rad_eff2(i));
...
for i=1:NL
    ...
    [clf_12(i),clf_21(i),~]=sandwich_panel_line(omega(i),B,Bf,G,mpa,hc,Lc,Ap1,Ap2);
...
disp(' ');
out1=sprintf(' Subsystem 1  %8.4g Hz',fring1);
out2=sprintf(' Subsystem 2  %8.4g Hz',fring2);
disp(out1);
disp(out2);
...
out2=sprintf('               = %8.4g lbm/ft^2',mpa*0.20485 );
out3=sprintf('               = %8.4g lbm/in^2',mpa*0.0014226 );
disp(out1);
disp(out2);
disp(out3);
...
handles.s=SEA_honeycomb_sandwich;
set(handles.s,'Visible','on');

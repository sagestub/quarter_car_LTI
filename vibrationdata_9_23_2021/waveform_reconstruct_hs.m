...
%
disp(' ');
disp(' waveform_reconstruct_hs.m   version 1.3   February 10, 2010');
...
%
disp(' ');		
nt=input(' Enter the number of trials per frequency. (suggest > 1000 ');
disp(' ');		
nfr=input(' Enter the number of frequencies. (suggest a few)');
%
if(nfr>MAXP)
    nfr=MAXP;
end
...
%
disp(' ');
kjv_num=input(' Enter the number of cases to minimize displacement (suggest >= 10)');
%
for(kjv=1:kjv_num)
%
...
%
        dmax=max(abs(ddd))
        if( dmax<drecord && error_max < error_all) 
            error_all=error_max
            drecord=dmax
            aaa_r=aaa;
            vvv_r=vvv;

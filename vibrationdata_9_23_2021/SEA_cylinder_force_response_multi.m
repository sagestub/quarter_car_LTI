function varargout = SEA_cylinder_force_response_multi(varargin)
% SEA_CYLINDER_FORCE_RESPONSE_MULTI MATLAB code for SEA_cylinder_force_response_multi.fig
%      SEA_CYLINDER_FORCE_RESPONSE_MULTI, by itself, creates a new SEA_CYLINDER_FORCE_RESPONSE_MULTI or raises the existing
%      singleton*.
%
%      H = SEA_CYLINDER_FORCE_RESPONSE_MULTI returns the handle to a new SEA_CYLINDER_FORCE_RESPONSE_MULTI or the handle to
%      the existing singleton*.
%
%      SEA_CYLINDER_FORCE_RESPONSE_MULTI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEA_CYLINDER_FORCE_RESPONSE_MULTI.M with the given input arguments.
%
%      SEA_CYLINDER_FORCE_RESPONSE_MULTI('Property','Value',...) creates a new SEA_CYLINDER_FORCE_RESPONSE_MULTI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SEA_cylinder_force_response_multi_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SEA_cylinder_force_response_multi_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SEA_cylinder_force_response_multi

% Last Modified by GUIDE v2.5 15-Jan-2016 18:00:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SEA_cylinder_force_response_multi_OpeningFcn, ...
                   'gui_OutputFcn',  @SEA_cylinder_force_response_multi_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SEA_cylinder_force_response_multi is made visible.
function SEA_cylinder_force_response_multi_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SEA_cylinder_force_response_multi (see VARARGIN)

% Choose default command line output for SEA_cylinder_force_response_multi
...
% UIWAIT makes SEA_cylinder_force_response_multi wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SEA_cylinder_force_response_multi_OutputFcn(hObject, eventdata, handles) 
...
delete(SEA_cylinder_force_response_multi);
...
for i=1:NL
    ...
    [fring,~,Y,~]=cylinder_mobility(em,rho,diam,L,thick,fc(i));
    
    power(i)=force(i)^2*real(Y);
    v2=power(i)/(mass*lf(i)*omega);
    vel(i)=sqrt(v2);
    accel(i)=vel(i)*omega;
    
    [mph(i)] = cylinder_mdens(fc(i),fring,L,thick);    
    Moverlap(i)=mph(i)*lf(i)*fc(i);  
end
...
for i=1:NL
    out1=sprintf('  %g %7.4g  %8.4g   %8.4g  %8.4g  %7.3g',...
                             fc(i),force(i),lf(i),vel(i),accel(i),Moverlap(i));
    disp(out1);
end
...
% --- Executes on button press in pushbutton_PFV.
function pushbutton_PFV_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_PFV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

A = imread('velox_force_alt.jpg');
...
% --- Executes on selection change in listbox_band.
function listbox_nband_Callback(hObject, eventdata, handles)
% hObject    handle to listbox_band (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function listbox_nband_CreateFcn(hObject, eventdata, handles)
% hObject    handle to listbox_band (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

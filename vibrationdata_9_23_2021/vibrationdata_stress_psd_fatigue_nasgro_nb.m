function varargout = vibrationdata_stress_psd_fatigue_nasgro_nb(varargin)
% VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB MATLAB code for vibrationdata_stress_psd_fatigue_nasgro_nb.fig
%      VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB, by itself, creates a new VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB returns the handle to a new VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB.M with the given input arguments.
%
%      VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB('Property','Value',...) creates a new VIBRATIONDATA_STRESS_PSD_FATIGUE_NASGRO_NB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_stress_psd_fatigue_nasgro_nb_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_stress_psd_fatigue_nasgro_nb_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_stress_psd_fatigue_nasgro_nb

% Last Modified by GUIDE v2.5 01-Mar-2016 18:17:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_stress_psd_fatigue_nasgro_nb_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_stress_psd_fatigue_nasgro_nb_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_stress_psd_fatigue_nasgro_nb is made visible.
function vibrationdata_stress_psd_fatigue_nasgro_nb_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_stress_psd_fatigue_nasgro_nb (see VARARGIN)

% Choose default command line output for vibrationdata_stress_psd_fatigue_nasgro_nb
...
% UIWAIT makes vibrationdata_stress_psd_fatigue_nasgro_nb wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_stress_psd_fatigue_nasgro_nb_OutputFcn(hObject, eventdata, handles) 
...
assignin('base', 'Dirlik_N',[ range/2 Dirlik_N*EP_tau/ds ]);
...
[Narrowband_N,range]=Dirlik_nasgro_probability_nb(n,m0,Q,D1,D2,D3,ds,Rd);

assignin('base', 'Narrowband_N',[ range/2 Narrowband_N*EP_tau/ds ]);


vo_tau=vo*tau;  % important for narrowband

[Narrowband_damage]=Dirlik_nasgro(n,A,B,C,P,R,vo_tau,Narrowband_N,range);
...
Seqrms=sigma_s*(1-R)^P;


lambda_ll=M2m^(m/2)/(vo*sigma_s^m);
LL_damage=Narrowband_damage*lambda_ll;

disp(' ');
out1=sprintf('\n        Dirlik Damage = %8.4g ',Dirlik_damage);
out2=sprintf(' Single-Moment Damage = %8.4g ',LL_damage);
disp(out1);
disp(out2);

disp(' ');
out1=sprintf('\n        Dirlik Damage Rate = %8.4g per sec',Dirlik_damage/tau);
out2=sprintf(' Single-Moment Damage Rate = %8.4g per sec',LL_damage/tau);
disp(out1);
disp(out2);

function varargout = vibrationdata_wavelet_SDOF_decomposition_alt(varargin)
% VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT MATLAB code for vibrationdata_wavelet_SDOF_decomposition_alt.fig
%      VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT, by itself, creates a new VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT returns the handle to a new VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT.M with the given input arguments.
%
%      VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT('Property','Value',...) creates a new VIBRATIONDATA_WAVELET_SDOF_DECOMPOSITION_ALT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_wavelet_SDOF_decomposition_alt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_wavelet_SDOF_decomposition_alt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_wavelet_SDOF_decomposition_alt

% Last Modified by GUIDE v2.5 14-May-2018 14:19:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_wavelet_SDOF_decomposition_alt_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_wavelet_SDOF_decomposition_alt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_wavelet_SDOF_decomposition_alt is made visible.
function vibrationdata_wavelet_SDOF_decomposition_alt_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_wavelet_SDOF_decomposition_alt (see VARARGIN)

% Choose default command line output for vibrationdata_wavelet_SDOF_decomposition_alt
...
% UIWAIT makes vibrationdata_wavelet_SDOF_decomposition_alt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_wavelet_SDOF_decomposition_alt_OutputFcn(hObject, eventdata, handles) 
...
%%%%
    ...
    for j=1:nbase
        ...
        amp(j)=peak*(-0.5+1.0*rand())/(10*nbase);
...
%    xlabel('Time (sec)');
    ...
    for kv=1:500
        ...
        if(kv>=2)
            ...
            amp=ampx*scale;
           y_resp=y_respx*scale;       
    
           
        error=std(input_accel-y_resp); 
        
         if(kv>=2 && error>error_before)
             break;
         end
    ...
    if(error<error_max)
        ...
        out1=sprintf('\n ix=%d  i=%d  kv=%d  scale=%7.3g  %11.7g   ',ix,i,kv,scale,error_max);
        disp(out1);
...
for j=1:nbase
    out1=sprintf(' %8.4g  %8.4g    %d   %8.4g  ',freq_r(j),amp_r(j),nhs_r(j),delay_r(j));
    disp(out1);
end

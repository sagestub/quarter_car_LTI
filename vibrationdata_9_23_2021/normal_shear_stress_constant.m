function varargout = normal_shear_stress_constant(varargin)
% NORMAL_SHEAR_STRESS_CONSTANT MATLAB code for normal_shear_stress_constant.fig
%      NORMAL_SHEAR_STRESS_CONSTANT, by itself, creates a new NORMAL_SHEAR_STRESS_CONSTANT or raises the existing
%      singleton*.
%
%      H = NORMAL_SHEAR_STRESS_CONSTANT returns the handle to a new NORMAL_SHEAR_STRESS_CONSTANT or the handle to
%      the existing singleton*.
%
%      NORMAL_SHEAR_STRESS_CONSTANT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NORMAL_SHEAR_STRESS_CONSTANT.M with the given input arguments.
%
%      NORMAL_SHEAR_STRESS_CONSTANT('Property','Value',...) creates a new NORMAL_SHEAR_STRESS_CONSTANT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before normal_shear_stress_constant_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to normal_shear_stress_constant_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help normal_shear_stress_constant

% Last Modified by GUIDE v2.5 23-May-2017 13:44:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @normal_shear_stress_constant_OpeningFcn, ...
                   'gui_OutputFcn',  @normal_shear_stress_constant_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before normal_shear_stress_constant is made visible.
function normal_shear_stress_constant_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to normal_shear_stress_constant (see VARARGIN)

% Choose default command line output for normal_shear_stress_constant
...
% UIWAIT makes normal_shear_stress_constant wait for user response (see UIRESUME)
...
function varargout = normal_shear_stress_constant_OutputFcn(hObject, eventdata, handles) 
...
delete(normal_shear_stress_constant);
...
px=stress_normal*scf_normal/Se_normal;
py=stress_shear*scf_shear/Se_shear;
...
gp_tau_ratio=zeros((nnn+1),1);
di_tau_ratio=zeros((nnn+1),1);
sm_tau_ratio=zeros((nnn+1),1);

for i=1:(nnn+1);
    
    sigma_ratio(i)=(i-1)*dx;
    
    gp_tau_ratio(i) = sqrt( 1-(sigma_ratio(i))^2 );
  
    di_tau_ratio(i) = sqrt( 1-(sigma_ratio(i))^k ); 
    
    sm_tau_ratio(i) = sqrt( 1-(k-1)*(sigma_ratio(i))^2 - (2-k)*(sigma_ratio(i)) ); 
...
plot(sigma_ratio,gp_tau_ratio,'k',...
     sigma_ratio,di_tau_ratio,'b',...
     sigma_ratio,sm_tau_ratio,'r'); 
...
legend( 'Gough-Pollard',  'Dietmann' , 'Socie-Marquis','Stress Case'); 
 
grid on;
xlabel('Normal Stress Ratio');
ylabel('Shear Stress Ratio');
out1=sprintf('Damage Curves, Fully-Reversed, in-phase, %g Cycles',nc);
title(out1);
xlim([0 1.2]);
ylim([0 1.2]);

%
%  circular_SS_base_half_sine.m  ver 1.0  February 28, 2012
%
%
function[fig_num]=circular_SS_base_half_sine(iu,fn,damp,NT,nodal_diam,PF,Cc,Dc,fig_num)
%        
    tpi=2*pi;
    disp(' ');
...
%
               ...
               kr=0; 
               nn=nodal_diam(j);
               ZZ=Cc(j)*( besselj(nn,kr)- Dc(j)*besseli(nn,kr))*cos(theta);                 
%
                rd(i)=rd(i) +ZZ*n(j);
               acc(i)=acc(i)+ZZ*na(j); 
%                           
...
%
    plot(t,rd);
%
    title('Relative Displacement at Center');
    if(iu==1)
        ylabel('Rel Disp(in) ');
    else
        ylabel('Rel Disp(m) ');

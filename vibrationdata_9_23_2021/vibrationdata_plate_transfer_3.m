%
%  vibrationdata_plate_transfer_3.m  ver 1.1   by Tom Irvine
%
...
%
    ...
    HM_stress_Tr=zeros(nf,1);    
    
    for k=1:nf
        
        HM_stress_vM(k)=sqrt( Hsxx(k)^2 + Hsyy(k)^2 - Hsxx(k)*Hsyy(k) + 3*Htxy(k)^2 );
        
        term1= ( Hsxx(k) + Hsyy(k) )/2;
        
        aaa= ((  Hsxx(k) - Hsyy(k) )/2)^2;
        
        term2=  sqrt(  aaa  +   Htxy(k)^2  );

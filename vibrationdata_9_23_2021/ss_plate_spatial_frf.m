disp(' ');
disp(' ss_plate_spatial_pressure_frf.m  ver 1.0  July 3, 2012');
disp(' by Tom Irvine  Email: tom@vibrationdata.com ');
disp(' ');
disp(' This script calculates the response of a rectangular plate  ');
disp(' simply-supported on all sides to a pressure field with      ');
disp(' the shape of the fundamental mode.                          ');
%
close all;
%
clear root;
...
for k=1:nf
    ...
    HM_stress_vM(k)=0;
%
    CC=cos(pi*x/a)*cos(pi*y/b);
    SS=sin(pi*x/a)*sin(pi*y/b);
%            
%            
    num=SS;
%            
    den=(omegamn(1,1)^2-omega(k)^2) + j*2*damp*omega(k)*omegamn(1,1);
%
    HM(k)=HM(k)+num/den;
%
    dxx=-(pi/a)^2*SS;

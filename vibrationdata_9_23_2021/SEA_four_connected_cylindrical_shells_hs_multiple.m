function varargout = SEA_four_connected_cylindrical_shells_hs_multiple(varargin)
% SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE MATLAB code for SEA_four_connected_cylindrical_shells_hs_multiple.fig
%      SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE, by itself, creates a new SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE or raises the existing
%      singleton*.
%
%      H = SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE returns the handle to a new SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE or the handle to
%      the existing singleton*.
%
%      SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE.M with the given input arguments.
%
%      SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE('Property','Value',...) creates a new SEA_FOUR_CONNECTED_CYLINDRICAL_SHELLS_HS_MULTIPLE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SEA_four_connected_cylindrical_shells_hs_multiple_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SEA_four_connected_cylindrical_shells_hs_multiple_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SEA_four_connected_cylindrical_shells_hs_multiple

% Last Modified by GUIDE v2.5 13-Jul-2018 16:46:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SEA_four_connected_cylindrical_shells_hs_multiple_OpeningFcn, ...
                   'gui_OutputFcn',  @SEA_four_connected_cylindrical_shells_hs_multiple_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SEA_four_connected_cylindrical_shells_hs_multiple is made visible.
function SEA_four_connected_cylindrical_shells_hs_multiple_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SEA_four_connected_cylindrical_shells_hs_multiple (see VARARGIN)

% Choose default command line output for SEA_four_connected_cylindrical_shells_hs_multiple
...
% UIWAIT makes SEA_four_connected_cylindrical_shells_hs_multiple wait for user response (see UIRESUME)
...
function varargout = SEA_four_connected_cylindrical_shells_hs_multiple_OutputFcn(hObject, eventdata, handles) 
...
delete(SEA_four_connected_cylindrical_shells_hs_multiple);
...
spl_name_1=strtrim(char(AA{1,2}));
spl_name_2=strtrim(char(AA{2,2}));
spl_name_3=strtrim(char(AA{3,2}));
spl_name_4=strtrim(char(AA{4,2}));


try
    THM1=evalin('base',SPL_name_1);
catch
    warndlg('SPL_name_1 not found ');
    return;
end
try
    THM2=evalin('base',SPL_name_2);
catch
    warndlg('SPL_name_2 not found ');
    return;
end
try
    THM3=evalin('base',SPL_name_3);
catch
    warndlg('SPL_name_3 not found ');
    return;
end
try
    THM4=evalin('base',SPL_name_4);
catch
    warndlg('SPL_name_4 not found ');
    return;
end
...
if(sz1(1)~=sz2(1) || sz1(1)~=sz3(1) || sz1(1)~=sz4(1) )
    warndlg('Input SPLs must have same number of rows');
    return;
end
if(sz1(2)~=sz2(2) || sz1(2)~=sz3(2) || sz1(2)~=sz4(2) )
    warndlg('Input SPLs must have same number of columns');
    return;
end

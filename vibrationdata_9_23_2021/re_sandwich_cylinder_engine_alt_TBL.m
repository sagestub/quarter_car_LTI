%
%  re_sandwich_cylinder_engine_alt_TBL.m  ver 1.1  by Tom Irvine
%
function[ffb,powerb]=...
        re_sandwich_cylinder_engine_alt_TBL(D,K,v,diam,L,mpa,bc,mmax,nmax,air_c,fcr,fring,freq,Ap,Uc,a1,a2)
...
powerb=zeros(length(ffb),1);

%
reff=zeros([1 kv]);
%
disp(' ')
disp('   fn(Hz)    n   m    kn     km    kair    re   type   cb   power');
%
mmm=kv-1;
%
j=1;
...
%
    ...
    if(i<250)
        out1=sprintf('%d  %8.2f  %d  %d  %6.2g  %6.2g %6.2g %6.2g %s %7.3g %7.3g',i,ffb(i),nnb(i),mmb(i),kn,km,k,reff(i),mtype,cw,powerb(i));
        disp(out1)
    end
%    
    if(nnb(i)==0)

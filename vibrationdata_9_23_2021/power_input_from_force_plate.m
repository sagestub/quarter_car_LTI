function varargout = power_input_from_force_plate(varargin)
% POWER_INPUT_FROM_FORCE_PLATE MATLAB code for power_input_from_force_plate.fig
%      POWER_INPUT_FROM_FORCE_PLATE, by itself, creates a new POWER_INPUT_FROM_FORCE_PLATE or raises the existing
%      singleton*.
%
%      H = POWER_INPUT_FROM_FORCE_PLATE returns the handle to a new POWER_INPUT_FROM_FORCE_PLATE or the handle to
%      the existing singleton*.
%
%      POWER_INPUT_FROM_FORCE_PLATE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POWER_INPUT_FROM_FORCE_PLATE.M with the given input arguments.
%
%      POWER_INPUT_FROM_FORCE_PLATE('Property','Value',...) creates a new POWER_INPUT_FROM_FORCE_PLATE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before power_input_from_force_plate_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to power_input_from_force_plate_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help power_input_from_force_plate

% Last Modified by GUIDE v2.5 15-Dec-2015 10:55:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @power_input_from_force_plate_OpeningFcn, ...
                   'gui_OutputFcn',  @power_input_from_force_plate_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before power_input_from_force_plate is made visible.
function power_input_from_force_plate_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to power_input_from_force_plate (see VARARGIN)

% Choose default command line output for power_input_from_force_plate
...
% UIWAIT makes power_input_from_force_plate wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = power_input_from_force_plate_OutputFcn(hObject, eventdata, handles) 
...
delete(power_input_from_force_plate);
...
if(ns==2 || ns==3)
    ...
    if(ns==2) % Plate, Bending, Semi-infinite
        Z=3.5*term;
    end
    if(ns==3) % Plate, Bending, Infinite
        Z=8*term;
    end

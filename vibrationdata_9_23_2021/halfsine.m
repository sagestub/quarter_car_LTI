...
%
		...
		amin=+1.0e+12;
%
        [amax,amin,acc,rd,TT,yb]=time_calc(rd_initial,rv_initial,domegan,omegad,damp,omega,omegan,omega2,omegan2,nt,dt,dur,amp,amax,amin);
%
        if(iopt==2)
            fnn(ijk)=fn;
%
...
if( iopt==1)
 ...
 %
        ...
        if(choice==1)
            ...
            out5 = sprintf(' SDOF Response, fn=%g Hz, Q=%g,    %g G, %g sec Half-sine Base Input  ',fn,Q,amp,dur);
            title(out5);         
        end    
%
...
%
    ...
    if(choice==1)       
        ...
        out5 = sprintf(' SRS Q=%g    %g G, %g sec Half-sine Base Input  ',Q,amp,dur);
        title(out5);
        grid;
...
if(iopt==1)
    ...
    if(achoice==1) 
        plot_title = sprintf(' SDOF Response, fn=%g Hz, Q=%g,    %g G, %g sec Half-sine Base Input  ',fn,Q,amp,dur);        
        n=length(TT);
        nn=round(n/3);
        TT_an=TT(1:nn);
        yb_an=yb(1:nn);

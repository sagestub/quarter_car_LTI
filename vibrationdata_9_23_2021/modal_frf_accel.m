function varargout = modal_frf_accel(varargin)
% MODAL_FRF_ACCEL MATLAB code for modal_frf_accel.fig
%      MODAL_FRF_ACCEL, by itself, creates a new MODAL_FRF_ACCEL or raises the existing
%      singleton*.
%
%      H = MODAL_FRF_ACCEL returns the handle to a new MODAL_FRF_ACCEL or the handle to
%      the existing singleton*.
%
%      MODAL_FRF_ACCEL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MODAL_FRF_ACCEL.M with the given input arguments.
%
%      MODAL_FRF_ACCEL('Property','Value',...) creates a new MODAL_FRF_ACCEL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before modal_frf_accel_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to modal_frf_accel_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help modal_frf_accel

% Last Modified by GUIDE v2.5 11-Oct-2018 10:35:47

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @modal_frf_accel_OpeningFcn, ...
                   'gui_OutputFcn',  @modal_frf_accel_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before modal_frf_accel is made visible.
function modal_frf_accel_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to modal_frf_accel (see VARARGIN)

% Choose default command line output for modal_frf_accel
...
% UIWAIT makes modal_frf_accel wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = modal_frf_accel_OutputFcn(hObject, eventdata, handles) 
...
if(p==1)
    handles.s= modal_frf_ensemble_accel;
else
    handles.s= modal_frf_single_accel;
end
...
if(n==2 && m==1)
    ...
    set(handles.text_IAN_1,'String','Accel Base Input Array Name');
    set(handles.text_IAN_2,'String','Accel Response Array Name');    
end
if(m==2)
%
   if(n==1)
...
%
      [filename, pathname] = uigetfile('*.*','Select the base input time history file.');
      filename = fullfile(pathname, filename);

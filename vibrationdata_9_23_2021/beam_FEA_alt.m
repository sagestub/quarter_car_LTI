function varargout = beam_FEA_alt(varargin)
% BEAM_FEA_ALT MATLAB code for beam_FEA_alt.fig
%      BEAM_FEA_ALT, by itself, creates a new BEAM_FEA_ALT or raises the existing
%      singleton*.
%
%      H = BEAM_FEA_ALT returns the handle to a new BEAM_FEA_ALT or the handle to
%      the existing singleton*.
%
%      BEAM_FEA_ALT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BEAM_FEA_ALT.M with the given input arguments.
%
%      BEAM_FEA_ALT('Property','Value',...) creates a new BEAM_FEA_ALT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before beam_FEA_alt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to beam_FEA_alt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help beam_FEA_alt

% Last Modified by GUIDE v2.5 24-Feb-2017 16:17:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @beam_FEA_alt_OpeningFcn, ...
                   'gui_OutputFcn',  @beam_FEA_alt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before beam_FEA_alt is made visible.
function beam_FEA_alt_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to beam_FEA_alt (see VARARGIN)

% Choose default command line output for beam_FEA_alt
...
% UIWAIT makes beam_FEA_alt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = beam_FEA_alt_OutputFcn(hObject, eventdata, handles) 
...
if(length(fn)>=16)
    setappdata(0,'fn',fn(1:16));    
    setappdata(0,'ModeShapes',ModeShapes(:,1:16));    
else
    setappdata(0,'fn',fn);

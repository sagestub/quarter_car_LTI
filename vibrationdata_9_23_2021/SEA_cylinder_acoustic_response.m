function varargout = SEA_cylinder_acoustic_response(varargin)
% SEA_CYLINDER_ACOUSTIC_RESPONSE MATLAB code for SEA_cylinder_acoustic_response.fig
%      SEA_CYLINDER_ACOUSTIC_RESPONSE, by itself, creates a new SEA_CYLINDER_ACOUSTIC_RESPONSE or raises the existing
%      singleton*.
%
%      H = SEA_CYLINDER_ACOUSTIC_RESPONSE returns the handle to a new SEA_CYLINDER_ACOUSTIC_RESPONSE or the handle to
%      the existing singleton*.
%
%      SEA_CYLINDER_ACOUSTIC_RESPONSE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEA_CYLINDER_ACOUSTIC_RESPONSE.M with the given input arguments.
%
%      SEA_CYLINDER_ACOUSTIC_RESPONSE('Property','Value',...) creates a new SEA_CYLINDER_ACOUSTIC_RESPONSE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SEA_cylinder_acoustic_response_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SEA_cylinder_acoustic_response_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SEA_cylinder_acoustic_response

% Last Modified by GUIDE v2.5 13-Jan-2016 11:32:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SEA_cylinder_acoustic_response_OpeningFcn, ...
                   'gui_OutputFcn',  @SEA_cylinder_acoustic_response_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SEA_cylinder_acoustic_response is made visible.
function SEA_cylinder_acoustic_response_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SEA_cylinder_acoustic_response (see VARARGIN)

% Choose default command line output for SEA_cylinder_acoustic_response
...
% UIWAIT makes SEA_cylinder_acoustic_response wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SEA_cylinder_acoustic_response_OutputFcn(hObject, eventdata, handles) 
...
delete(SEA_cylinder_acoustic_response);
...
Slf = get(handles.edit_lf, 'String');
if isempty(Slf)
...
[rad_eff]=cylinder_rad_eff(f,fcr,fring,L,diam,c,mu);

[mph] = cylinder_mdens(f,fring,L,thick);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

[P]=equivalent_acoustic_power(mph,c,rad_eff,pressure,md,thick,freq);
...
[Moverlap]=SEA_modal_overlap(mph,lf,freq);
...
disp(' ');
disp('   Cylinder Spatial Average Response ');
disp(' ');

a=v*omega;

if(iu==1)
    out1=sprintf('     Velocity = %8.4g (in/sec) rms',v);
    out2=sprintf(' Acceleration = %8.4g G rms',a/386);    
else
    out1=sprintf(' Velocity = %8.4g mm/sec',v*1000);
    out2=sprintf(' Acceleration = %8.4g G rms',a/9.81);     
end
...
%% out1=sprintf(' p=%8.4g c=%8.4g gas_md=%8.4g M=%8.4g Ap=%8.4g lf=%8.4g',pressure,c,gas_md,M,Ap,lf);
%% disp(out1);

function varargout = multiaxis_fatigue_damage(varargin)
% MULTIAXIS_FATIGUE_DAMAGE MATLAB code for multiaxis_fatigue_damage.fig
%      MULTIAXIS_FATIGUE_DAMAGE, by itself, creates a new MULTIAXIS_FATIGUE_DAMAGE or raises the existing
%      singleton*.
%
%      H = MULTIAXIS_FATIGUE_DAMAGE returns the handle to a new MULTIAXIS_FATIGUE_DAMAGE or the handle to
%      the existing singleton*.
%
%      MULTIAXIS_FATIGUE_DAMAGE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MULTIAXIS_FATIGUE_DAMAGE.M with the given input arguments.
%
%      MULTIAXIS_FATIGUE_DAMAGE('Property','Value',...) creates a new MULTIAXIS_FATIGUE_DAMAGE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before multiaxis_fatigue_damage_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to multiaxis_fatigue_damage_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help multiaxis_fatigue_damage

% Last Modified by GUIDE v2.5 07-Mar-2016 15:56:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @multiaxis_fatigue_damage_OpeningFcn, ...
                   'gui_OutputFcn',  @multiaxis_fatigue_damage_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before multiaxis_fatigue_damage is made visible.
function multiaxis_fatigue_damage_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to multiaxis_fatigue_damage (see VARARGIN)

% Choose default command line output for multiaxis_fatigue_damage
...
% UIWAIT makes multiaxis_fatigue_damage wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = multiaxis_fatigue_damage_OutputFcn(hObject, eventdata, handles) 
...
n_sn_curve_type=get(handles.listbox_sn_curve_type,'Value');
...
if(iu==1)  % convert psi to ksi
    THM(:,2:ncols)=THM(:,2:ncols)/1000;
    sx=THM(:,2);
    sy=THM(:,3);
    txy=THM(:,4);
end
if(iu==3)  % convert Pa to ksi
    THM(:,2:ncols)=THM(:,2:ncols)*1.450e-7;  
    sx=THM(:,2);
    sy=THM(:,3);
    sz=THM(:,4);  
    txy=THM(:,5);
...
if(n_stress_format==2)
    ...
    for i=1:NT
        ...
        A=[sx txy txz; txy sy tyz; txz tyz sz];
...
if(n_sn_curve_type==1)  % signed max abs principal
    
    for i=1:NT
    end
    
end
if(n_sn_curve_type==2)  % signed von Mises
end
if(n_sn_curve_type==3)  % signed Tresca
end
if(n_sn_curve_type==4)  % hypersphere
end
...
%%    
%%  0.5 factor converts from range to amp
%%
    for iv=1:sz(1)

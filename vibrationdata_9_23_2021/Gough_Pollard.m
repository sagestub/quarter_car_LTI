function varargout = Gough_Pollard(varargin)
% GOUGH_POLLARD MATLAB code for Gough_Pollard.fig
%      GOUGH_POLLARD, by itself, creates a new GOUGH_POLLARD or raises the existing
%      singleton*.
%
%      H = GOUGH_POLLARD returns the handle to a new GOUGH_POLLARD or the handle to
%      the existing singleton*.
%
%      GOUGH_POLLARD('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GOUGH_POLLARD.M with the given input arguments.
%
%      GOUGH_POLLARD('Property','Value',...) creates a new GOUGH_POLLARD or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Gough_Pollard_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Gough_Pollard_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Gough_Pollard

% Last Modified by GUIDE v2.5 12-May-2017 18:07:53

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Gough_Pollard_OpeningFcn, ...
                   'gui_OutputFcn',  @Gough_Pollard_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Gough_Pollard is made visible.
function Gough_Pollard_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Gough_Pollard (see VARARGIN)

% Choose default command line output for Gough_Pollard
...
% UIWAIT makes Gough_Pollard wait for user response (see UIRESUME)
...
function varargout = Gough_Pollard_OutputFcn(hObject, eventdata, handles) 
...
Sa_normal = (A/nc)^(1/m);
Sa_shear = Sa_normal*scale;


damage=(scf_normal*stress_normal/Sa_normal)^2 + (scf_shear*stress_shear/Sa_shear)^2;
...
out1=sprintf(' Normal Stress Limit  = %8.3g %s  for %8.4g cycles ',Sa_normal,su,nc);
disp(out1);
...
if(damage<1)
    out1=sprintf(' Gough-Pollard Damage = %8.3g < 1 ,  below fatigue limit\n',damage);
    disp(out1);
end

if(damage==1)
    out1=sprintf(' Gough-Pollard Damage = %8.3g = 1  , at fatigue limit\n',damage);
    disp(out1);
end

if(damage>1)
    out1=sprintf(' Gough-Pollard Damage = %8.3g > 1  , above fatigue limit\n',damage);
    disp(out1);
end

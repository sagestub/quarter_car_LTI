%
%  re_sandwich_cylinder_engine_alt_dlf.m  ver 1.0  by Tom Irvine
%
function[rad_eff,mph,fstart_rad]=...
...
%
    ...
    modal_overlap(j)=nun(j)*dlf(j)*fc(j);
...
for j=NL:-1:1
    ...
    if(modal_overlap(j)>=1 || nmodes(j) >=3)

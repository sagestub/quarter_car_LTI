%
disp(' twodof_half_sine_force.m   ver 1.2  January 20, 2012');
disp(' by Tom Irvine  Email: tomirvine@aol.com ');
disp(' ');
%
disp(' This script calculates the response of a two-degree-of-freedom ');
disp(' system to a half-sine force excitation at dof 2. ');
disp(' ');
%
close all;
%
...
%
    disp(' ');
    B(2)=   input(' Enter the force amplitude (lbf)  ');
    alpha(2) =input(' Enter the half-sine pulse duration (sec)   ');
%
    disp(' ');
...
%
    disp(' ');
    B(2)=   input(' Enter the force amplitude (N)  ');
    alpha(2) =input(' Enter the half-sine pulse duration (sec)   ');
%
    disp(' ');
...
dt=1/sr;
disp(' ');
dur=input(' Enter the analysis duration (sec) ');
%
nt=round(dur/dt);
%
clear n;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Solve for initial conditions at end of pulse
%
     for j=1:2  % nodal dof
%
        omdt=omegad(j)*T;
...
%           
             ndT(j)= -sum(n1)+sum(n2) + n3;
             nvT(j)=-sum(nv1)+sum(nv2)+nv3;
     end
%
for j=1:2
  AAT(j)=(nvT(j)+domegan(j)*ndT(j))/omegad(j); 
end         
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
            ...
            naccel(j)=0.;
%
          ee=exp(-domegan(j)*tau);
         dee=-domegan(j)*ee;
        ddee=(domegan(j))^2*ee;
%
            n3= ee*(ndT(j)*cos_odt + AAT(j)*sin_odt);
%
          nv3=           dee*( ndT(j)*cos_odt + AAT(j)*sin_odt)+...
                omegad(j)*ee*(-ndT(j)*sin_odt + AAT(j)*cos_odt);
%
        na3=-2*domegan(j)*nv3-omegan2(j)*n3;
%
             n(j,i)=  n3;

...
//   rainflow_mean_mex.cpp  ver 1.8  May 2, 2017
...
void rainflow_cpp(double *L,double *C,double *AverageAmp,double *MaxAmp,
                     ...
                     double *D,vector<float> &tac1,vector<float> &tac2,vector<float> &tamean,
...
//        mexPrintf("%8.4g \t %8.4g \n",Y,X);
    	...
    	if(X>=Y && Y>0)
		{
        	...
        	if(i==0)
			{
                ...
                B[kv][1]=0.5;
                B[kv][0]=Y;
                tac1.push_back(0.5*B[kv][0]);
                tac2.push_back(B[kv][1]);   
                tamean.push_back((B[kv][3]+B[kv][2])/2.);
...
//*************************************************************************
    ...
    rainflow_cpp(L,C,AverageAmp,MaxAmp,MinMean,AverageMean,MaxMean,MinValley,MaxPeak,D,tac1,tac2,tamean,cL,y,dchoice,exponent,num);
...
//*************************************************************************
    ...
    plhs[10]= mxCreateDoubleMatrix(kv, 1, mxREAL);
    plhs[11]= mxCreateDoubleMatrix(kv, 1, mxREAL);
    plhs[12]= mxCreateDoubleMatrix(kv, 1, mxREAL);    

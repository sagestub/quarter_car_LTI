%
%    sf_engine3_ndel.m  ver 2.2  by Tom Irvine
%
function[a,x1r,x2r,x3r,x4r,x5r,cnew,errormax] = ...
    sf_engine3(a,t,num2,flow,fup,ie,nt,dur,x1r,x2r,x3r,x4r,...
                  x5r,running_sum,original,istr,max_damp,min_damp,errormax,ndel)
%
tpi=2*pi;
%
...
%
        if(tt>x5)
            ta=tt-x5;
			yr(i)=exp(-domegan*ta)*sin(omegad*ta+x3);
        end
        
    end
    
    [x1]=damped_sine_lsf_function(a,yr);
    
    yr=x1*yr;

    error=0;
    for i=1:n
        error=error+abs(a(i)-yr(i));
    end
